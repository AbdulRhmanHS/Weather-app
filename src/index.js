...
async function getWeatherData(location, units = 'metric') {
  try {
    ...
    const url = `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${encodeURIComponent(location)}?unitGroup=${units}&key=${API_KEY}&contentType=json`;
...
async function showWeather(userLocation, units = 'metric') {
  let weatherData = await getWeatherData(userLocation, units);
...
function changeUnits(userLocation) {
  document.addEventListener('click', async (e) => {
    if (e.target.classList.contains('units-btn')) {
      let currentSystem = getCurrentSystem();
      currentSystem = currentSystem === 'metric' ? 'us' : 'metric';
      setCurrentSystem(currentSystem);
      await showWeather(userLocation, currentSystem);
    }
  });
}
...
(async () => {
  ...
  if (userLocation) {
    await showWeather(userLocation);
    changeUnits(userLocation);
  }
})();
